...
theme_update(plot.margin = margin(30, 30, 30, 30),
             ...
             legend.background = element_rect(fill = "white",
                                              color = "#eaeaea"),
             legend.key = element_rect(color = "#eaeaea", fill = "white"),
...
theme_rmdcustom <- 
  function(){
  ggthemes::theme_clean(base_size = 12, base_family = "Ubuntu")+
...
youtube_data %>% count(month) %>% 
  ...
  labs(subtitle = "YouTube data",y="Number of videos",x="Month")+
  theme_rmdcustom()
...
ggplot(dates_views_comments)+
  geom_point(aes(x=pub_date,y=views,fill=source),color="black",pch=21)+
  labs(y="Views",x="Publication date")+theme_rmdcustom()+
  scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")

ggplot(dates_views_comments)+
  geom_point(aes(x=pub_date,y=comments,fill=source),color="black",pch=21)+
    labs(x="Publication date",y="Comments")+theme_rmdcustom()+
    scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")

ggplot(dates_views_comments)+
  geom_point(aes(x=views,y=comments,fill=source),color="black",pch=21)+
    labs(x="Views",y="Comments")+theme_rmdcustom()+
    scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")

ggplot(dates_views_comments)+
  geom_point(aes(x=followers,y=comments,fill=source),color="black",pch=21)+
    labs(x="Followers",y="Comments")+theme_rmdcustom()+
    scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")

dates_views_comments %>% filter(followers!=44100000) %>% 
ggplot()+
  geom_point(aes(x=followers,y=comments,fill=source),color="black",pch=21)+
    labs(x="Followers",y="Comments",subtitle = "Outliers removed")+theme_rmdcustom()+
    scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")

dates_views_comments %>% filter(followers!=44100000) %>% 
ggplot()+
  geom_point(aes(x=followers,y=views,fill=source),color="black",pch=21)+
    labs(x="Followers",y="Views",subtitle = "Outliers removed")+theme_rmdcustom()+
    scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")
```
...
tt_ages <- tiktok_data %>% tabyl(presenter_age) %>% na.omit() %>% select(-percent) %>% mutate(percent=round(valid_percent,2)) %>% select(-valid_percent)%>% mutate(source="TikTok")
...
```{r}
  gendplt <- 
    ...
    geom_bar_text(family="Ubuntu",fullheight = T,size=23)+
    scale_fill_pretty_d(name = "Relax",guide="none")+
...
ageplt <- 
  ...
  geom_bar_text(position = "dodge",family="Ubuntu")+  
  labs(y="Number of videos",x="Presenter age",
       subtitle="<span style = 'color: #69c9d0; font-family:Montserrat'>TikTok</span> & <span style = 'color: #FF0000; font-family: Oswald'>YouTube</span>",
         caption = "Excludes text-to-speech or voice-over presenters")+
    scale_fill_manual(values = c("#69c9d0","#FF0000"),guide="none")+
    scale_color_manual(values=c("#ff0050","#282828"),guide="none")+
...
# for export
ageplt+gendplt+plot_annotation(title="Presenter Demographics",theme = 
  theme(plot.title=element_text(color="black")))

ggsave(here::here("plots", "presenter_demographics.png"), 
          width = 12.5, height = 8, dpi = "retina")
...
- Longer time frames report higher median earnings (per 1 unit of time, **_not standardized_**)
...
bind_rows(yt_hourly_earn,tt_hourly_earn) %>% 
  filter(hourly_earn<1300) %>% 
  ggplot()+
  geom_histogram(aes(hourly_earn,fill=source,color=source),
                 linewidth=0.6,bins=25,position = "dodge")+
  annotate(
    geom = "curve", x = 810, y = 7, xend = 960, yend = 1, 
    curvature = .1, arrow = arrow(length = unit(2, "mm"))
  ) +
  annotate(geom = "text", x = 360, y = 12,family="Ubuntu",size=5,
           label = "Not shown:\nTwo videos suggesting $1500 and $1800 in earnings\n(one from YouTube, one from TikTok)", hjust = "left")+
  labs(x="Hourly earnings (USD)",
       y="Number of money-making activities",
       title="Distribution of Hourly Earnings by Platform",
       caption = "Videos often included >1 activites",
       subtitle = "<span style = 'color: #69c9d0; font-family:Montserrat'>TikTok</span> &  <span style = 'color: #FF0000; font-family: Oswald'>YouTube</span>")+
  scale_fill_manual(values =c("#69c9d0","#FF0000"),guide="none")+
  scale_x_continuous(labels=scales::dollar_format(),expand = c(0.01,0))+
  scale_y_continuous(expand = c(0,0))+
  scale_color_manual(values=c("#ff0050","#282828"),guide="none")+
  theme(plot.title = element_text(color = "black"),
        plot.subtitle = element_markdown(hjust = 1,face = "bold"),
        axis.line.y = element_blank(),
        legend.position = c(0.8,0.8),text = element_text(family='Ubuntu'))

ggsave(here::here("plots", "earnings_by_platform.png"), 
          width = 12.5, height = 8, dpi = "retina")
...
bind_rows(yt_hourly_earn,tt_hourly_earn) %>% 
  ...
  facet_wrap(~earnings_timeframe,scales = 'free',ncol = 2)+
  labs(x="Earnings (USD",y="Number of activities")+
  scale_x_continuous(labels=scales::dollar_format())+
  theme_rmdcustom()+theme(panel.spacing.x = unit(0.29, "lines"))
...
yt_hourlycorrs_bus2 %>% group_by(yt_video_id,idea) %>% 
  ...
  coord_flip()+labs(x="Business Type Level 2",y="Hourly earnings (USD)")+theme_rmdcustom()
...
ggplot(hourlyboth)+aes(x=views,y=earn,fill=source)+geom_point(color="black",pch=21)+
  theme_rmdcustom()+labs(x="Views",y="Earnings (USD)")+
  scale_y_continuous(labels = scales::dollar_format())+
  scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")

ggplot(hourlyboth)+aes(x=comments,y=earn,fill=source)+
  geom_point(color="black",pch=21)+
  theme_rmdcustom()+labs(x="Comments",y="Earnings (USD)")+
  scale_y_continuous(labels = scales::dollar_format())+
  scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")

ggplot(hourlyboth)+
  aes(x=followers,y=earn,fill=source)+geom_point(color="black",pch=21)+
  theme_rmdcustom()+labs(x="Followers",y="Earnings (USD)")+
  scale_y_continuous(labels = scales::dollar_format())+
    scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")
```
...
tt_hourlycorrs_bus2 %>% group_by(tt_video_id,idea) %>% 
  ...
  geom_sina(aes(x=str_wrap(business_type_level_2,33),
                  y=hourly_earn,color=business_type_level_2))+
  scale_color_discrete(guide="none")+labs(x="Business Type (level 1)")+
  labs(y="Hourly earnings (USD)")+
  scale_y_continuous(labels = scales::dollar_format())+
  coord_flip()+labs(x="Business Type Level 2")+theme_rmdcustom()

```

### Share of business types (Level 1) by platform
```{r}
bus1ct <- bus1ct %>% na.omit %>% mutate(source="YouTube")
tt_bus1ct <- tt_bus1ct %>% na.omit %>% mutate(source="TikTok")

xlabsdf <- bind_rows(bus1ct,tt_bus1ct) %>% distinct(business_type_level_1) %>% mutate(x=1:5,y=1)


bind_rows(bus1ct,tt_bus1ct) %>% 
ggplot(aes(x=fct_reorder(business_type_level_1,n),y=n,fill=source,
               color=source,group=source))+
  geom_col(linewidth=0.7,
           position = "dodge",width = 0.8)+
  labs(y="Number of mentions",
       title="Business Types Mentioned by Platform",
       subtitle = "<span style = 'color: #69c9d0; font-family:Montserrat'>TikTok</span> &  <span style = 'color: #FF0000; font-family: Oswald'>YouTube</span>")+
  geom_bar_text(position = "dodge",reflow = TRUE,family="Ubuntu",size=16)+
  scale_y_continuous(expand = c(0,0))+
  scale_fill_manual(values = c("#69c9d0","#FF0000"),guide='none')+
  scale_color_manual(values=c("#ff0050","#282828"),guide='none')+
  theme(plot.title = element_text(color = "black"),
...
# export
ggsave(here::here("plots", "business_types_platform.png"), 
          width = 12.5, height = 8, dpi = "retina")

```


### Share of business types (Level 2) by platform

```{r, fig.width=15}
bus2ctp <- bus2ct %>% na.omit %>% mutate(source="YouTube")%>% 
  separate(business_type_level_2,into=c("level1","level2"),sep=" - ") %>% mutate(across(c(level1,level2),str_squish)) 
bus2cttp<- bus2ctt %>% na.omit %>% mutate(source="TikTok")%>% 
  separate(business_type_level_2,into=c("level1","level2"),sep=" - ") %>% mutate(across(c(level1,level2),str_squish)) 

bus2filttt <- 
bus2cttp %>% 
  mutate(level2=if_else(n<4,paste0("Miscellaneous - ",level1),level2)) %>% 
  group_by(level1,level2) %>% summarise(newn=sum(n)) %>% filter(newn>1) %>% 
  ungroup() %>% mutate(level2text=str_remove(level2,"\\(.*\\)"))  
...
ggplot(bus2filttt,aes(x=fct_reorder(level2text,newn),y=newn,
                      fill=level1))+
  ...
  labs(x="Business type",
       ...
       title="Business Types Mentioned - <span style = 'color: #69c9d0; font-family:Montserrat'>TikTok</span>")+
  geom_bar_text(family="Ubuntu")+
  coord_flip()+
  scale_y_continuous(expand = c(0,0))+
  theme(plot.title = element_markdown(color="black"),
        legend.position = c(0.75,0.26),
        axis.line.x = element_blank(), 
        axis.title.y = element_blank(),
        legend.title = element_blank(),
        axis.text.y = element_text(size=14,family =" Ubuntu Thin" ),
        axis.text.x = element_text(size=16),
        panel.grid.major.x = element_blank(),
        panel.grid.major.y = element_blank(),
        text = element_text(family='Ubuntu'))

ggsave(here::here("plots", "tt_business_types.png"), 
          width = 12.5, height = 8, dpi = "retina")
```
...
ggplot(bus2filtyt,aes(x=fct_reorder(level2text,newn),y=newn,
                      fill=level1))+
  geom_col(linewidth=0.3,
  ...
  labs(x="Business Type",
       y="Number of mentions",
       title="Business Types Mentioned - <span style = 'color: #FF0000; font-family: Oswald'>YouTube</span>")+
  geom_bar_text(family="Ubuntu")+
  coord_flip()+
  scale_y_continuous(expand = c(0,0))+
  theme(plot.title = element_markdown(color="black"),
        legend.position = c(0.73,0.26),
        axis.line.x = element_blank(), 
        axis.title.y = element_blank(),
        legend.title = element_blank(),
        axis.text.y = element_text(size=14,family =" Ubuntu Thin" ),
...
ggsave(here::here("plots", "yt_business_types.png"), 
          width = 12.5, height = 8, dpi = "retina")
```


## Earnings per business type
...
bind_rows(ytbus1earn,ttbus1earn) %>% add_count(business_type_level_1) %>% 
  ggplot(aes(y=fct_reorder(business_type_level_1,n),
             x=hourly_earn,fill=fct_reorder(business_type_level_1,n)))+
stat_binline(scale=1.21,bins=41,size=0.3)+
  labs(x="Hourly earnings (USD)",
       title = "Hourly Earnings by Business Type")+
  scale_fill_pretty_d("Relax",guide="none")+
  scale_x_continuous(limits = c(-30,1000),expand = c(0,0),
...
ggsave(here::here("plots", "earnings_business_types.png"), 
          width = 12.5, height = 8, dpi = "retina")
```
...
ytbus2earn <- 
yt_hourlycorrs_bus2 %>% group_by(yt_video_id,idea) %>% 
  distinct(yt_video_id,idea,business_type_level_2,hourly_earn) %>% ungroup %>% mutate(source="YouTube") 

ttbus2earn <- 
tt_hourlycorrs_bus2 %>% group_by(tt_video_id,idea) %>% 
  distinct(tt_video_id,idea,business_type_level_2,hourly_earn) %>% ungroup %>% mutate(source="TikTok") 

bind_rows(ytbus2earn,ttbus2earn) %>% add_count(business_type_level_2) %>% 
    separate(business_type_level_2,into=c("level1","level2"),sep=" - ") %>% mutate(across(c(level1,level2),str_squish)) %>% filter(n>3) %>%  mutate(level2=str_remove(level2,"\\(.*\\)"))  %>% 
  ggplot(aes(y=fct_reorder(level2,n),
             x=hourly_earn,fill=level1))+
stat_binline(scale=1.21,bins=41,size=0.3)+
  labs(x="Hourly earnings (USD)",
       title = "Hourly Earnings by Business Type")+
  scale_fill_pretty_d("Relax")+
  ...
  scale_y_discrete(expand = c(0.02, 0)) +
    theme(axis.title.y = element_blank(),
          axis.text.y = element_text(family = "Ubuntu Thin"),
        axis.line.y = element_blank(),
        legend.title = element_blank(),
...
ggsave(here::here("plots", "earnings_business_types_level2.png"), 
          width = 12.5, height = 8, dpi = "retina")
```
...
ggplot()+
  ...
  theme_rmdcustom()+
  labs(x="Number of skills mentioned",y="Number of videos")+
  scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")
```
...
bind_rows(ytskearnsummary,ttskearnsummary) %>% 
  ggplot()+
  geom_bar(aes(x=fct_reorder(skills_required,mean_earn),y=mean_earn,fill=source),stat="identity",position = "dodge")+coord_flip()+
    theme_rmdcustom()+labs(x="Skills",y="Earnings (USD)")+
  scale_y_continuous(labels = scales::dollar_format())+
  scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")
```
...
bind_rows(ytskillearns,ttskillearns) %>% 
  ...
  geom_sina()+labs(x="Skills",y="Earnings (USD)")+
  coord_flip()+
  theme_rmdcustom()+
  scale_y_continuous(labels = scales::dollar_format())+
  scale_fill_manual(values=c("#69c9d0","#FF0000"),name="Video Source")
...
bind_rows(skillsearntt,skillsearnyt) %>% 
  ...
  scale_y_continuous(labels = scales::dollar_format())+
  labs(x="Number of skills required per money-making idea",y="Earnings (USD)")+theme_rmdcustom()
```
...
ggplot(top15wrds)+
  ...
  coord_flip()+theme_rmdcustom()+theme(panel.spacing.x = unit(0.26,"lines"))
